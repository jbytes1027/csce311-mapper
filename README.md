...
From the build directory, create a make file with cmake.

```
cmake ..
```

Then build by running the make file.

```
make
```

The default will generate two executables: `mapper` and `mapper-test`.

## Testing Mapper

To run the tests run `./mapper-test` or `ctest` from the build directory.

## Using Mapper

Run `./mapper [INPUT FILE...] [OUTPUT FILE...]`.
